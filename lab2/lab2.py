...
def free_space_path_loss(d, f):
    ...
    path_loss = 32.45 + 20 * np.log10(d) + 20 * np.log10(f)
    return path_loss
...
def free_space_received_power(transmitted_power, distance, frequency, tx_antenna_gain=14, rx_antenna_gain=14, cable_loss=4):
    received_power = transmitted_power - \
        free_space_path_loss(distance, frequency) + \
        tx_antenna_gain+rx_antenna_gain-cable_loss
    return received_power


def CCIR_received_power(transmitted_power, d, f, b, hb, hm, tx_antenna_gain=14, rx_antenna_gain=14, cable_loss=4):

    received_power = transmitted_power - CCIR_path_loss(d, f, b, hb, hm)+tx_antenna_gain + \
        rx_antenna_gain-cable_loss
    return received_power


def Hata_Model_urban_received_power(transmitted_power, f, h, a, d, C=0, tx_antenna_gain=14, rx_antenna_gain=14, cable_loss=4):

    received_power = transmitted_power - Hata_Model_urban(f, h, a, d, C=0)+tx_antenna_gain + \
        rx_antenna_gain-cable_loss
    return received_power


def Hata_Model_suburban_received_power(transmitted_power, f, h, a, d, C=0, tx_antenna_gain=14, rx_antenna_gain=14, cable_loss=4):

    received_power = transmitted_power - Hata_Model_suburban(f, h, a, d, C=0)+tx_antenna_gain + \
        rx_antenna_gain-cable_loss
    return received_power


def Hata_Model_rural_received_power(transmitted_power, f, h, a, d, C=0, tx_antenna_gain=14, rx_antenna_gain=14, cable_loss=4):

    received_power = transmitted_power - Hata_Model_rural(f, h, a, d, C=0)+tx_antenna_gain + \
        rx_antenna_gain-cable_loss
    return received_power
...
path_loss = free_space_path_loss(d, f[0])
path_loss_2 = free_space_path_loss(d, f[1])
...
plot_default("Free space CICR  Hata Models COMPARISON (urban, suburban, rural) for 900 and 1800 MHz")

fig11 = plt.figure(14)
plt.plot(d, free_space_received_power(39, d, f[0]), label='free space 900 MHz')
plt.plot(d, free_space_received_power(
    39, d, f[1]), label=' free space 1800 MHz')
plt.plot(d, CCIR_received_power(
    39, d, f[1], 0.25, 35, 1), label=' CICR 900 MHz')
plt.plot(d, CCIR_received_power(
    39, d, f[1], 0.25, 35, 1), label='CCIR 1800 MHz')

plt.plot(d, Hata_Model_urban_received_power(
    39, f[0], 35, Hata_Model_medium_city_correction_factor(900, hm), d), label='Hata urban 900 MHz')
plt.plot(d, Hata_Model_urban_received_power(
    39, f[0], 35, Hata_Model_medium_city_correction_factor(1800, hm), d), label='Hata urban 1800 MHz')

plt.plot(d, Hata_Model_suburban_received_power(
    39, f[1], 35, Hata_Model_large_city_corection_factor(900, hm), d), label='Hata suburban 1800 MHz')
plt.plot(d, Hata_Model_suburban_received_power(
    39, f[1], 35, Hata_Model_large_city_corection_factor(1800, hm), d), label='Hata suburban 1800 MHz')

plt.plot(d, Hata_Model_rural_received_power(
    39, f[1], 35, Hata_Model_medium_city_correction_factor(900, hm), d), label='Hata rural 900 MHz')
plt.plot(d, Hata_Model_rural_received_power(
    39, f[1], 35, Hata_Model_medium_city_correction_factor(1800, hm), d), label='Hata rural 1800 MHz')

plot_default(
    "Comparing received power of CICR, Free space, and Hata models in urban, suburban, and rural areas for 900 and 1800 MHz.",y_label="Received Power (dBm)")

fig15 = plt.figure(15)
plt.plot(d, Hata_Model_rural_received_power(
    39, f[1], 35, Hata_Model_medium_city_correction_factor(1800, hm), d), label='Hata rural 1800 MHz')
plot_default("Hata rural 1800 MHz", y_label="Received Power (dBm)")
...
# Make same as fig 12 but use hata class to plot
plt.show()
